...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-c", "--confidence_interval", type=float, default=50, help="The confidence interval to visualize.")
...
def main():
    ...
    ax2 = ax1.twinx()
    ax1 = sns.lineplot(data=interpolated_df, x="iteration", y="area", ax=ax1, legend=False, label="Area", errorbar=("ci", args.confidence_interval))
    ax2 = sns.lineplot(data=interpolated_df, x="iteration", y="error", ax=ax2, color="red", legend=False, label="Error", errorbar=("ci", args.confidence_interval))
